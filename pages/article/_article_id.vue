<template>
  <div class="post">
    <h3 class="date">{{ updateTime }}</h3>
    <div class="content" v-html="articleContent"></div>
    <a v-if="hasNewerArtical" id="newer" class="blog-nav" :href="newerArticalLink">&lt;&nbsp;NEWER</a>
    <a v-if="hasOlderArtical" id="older" class="blog-nav" :href="olderArticalLink">OLDER&nbsp;&gt;</a>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    article () {
      return this.list.data[this.articleId]
    },
    articleContent () {
      return Marked(this.article.body)
    },
    updateTime () {
      return this.article.updated_at.match(/\d{4}-\d{2}-\d{2}/)[0]
    },
    // 是否存在新文章
    hasNewerArtical () {
      return Number(this.articleId) < Number(this.list.data.length - 1)
    },
    // 是否存在旧文章
    hasOlderArtical () {
      return Number(this.articleId) > 0
    },
    // 新文章链接
    newerArticalLink () {
      return `/article/${Number(this.articleId) + 1}`
    },
    // 旧文章链接
    olderArticalLink () {
      return `/article/${Number(this.articleId) - 1}`
